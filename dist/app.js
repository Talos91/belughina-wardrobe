...
let tab='outfits',destination='wardrobe',wardrobeTab='outfits',colorTarget='base',part='primary',character,characterPicker,motion,grounding,contactShadows,eyeSkinTint,wardrobeAssets,yaw=-.13,targetYaw=-.13,soundEnabled=false,audioContext,speechTimer,toastTimer,photoSnapshot;
const outfitHistory=[];
function rememberLook(){outfitHistory.push(structuredClone(state));if(outfitHistory.length>30)outfitHistory.shift();}
...
function renderItems(){
 ...
 const list=tab==='room'?PLACES:tab==='dresses'?DRESSES:tab==='outfits'?CLOTHES:slot?CLOTHES.filter(x=>x.slot===slot):EXTRAS;
 ...
 $('#category-title').textContent={dresses:'Dresses',outfits:'Outfits',tops:'Tops',bottoms:'Bottoms',extras:'Accessories',room:'Choose a room'}[tab];
 ...
 box.replaceChildren();box.setAttribute('aria-labelledby',tab==='room'?'category-title':'tab-'+tab);
 box.classList.toggle('stacked-outfits',tab==='outfits');
 $('#clothes-slots').hidden=true;
 const empty=slot?!state[slot]:tab==='extras'?!state.extras.length:state.outfit==='base';
 $('#clear-outfit').textContent=slot?'No '+slot:tab==='extras'?'Clear extras':tab==='outfits'?'Clear outfit':'No dress';
 $('#clear-outfit').setAttribute('aria-pressed',String(empty));
 let section;
 for(const item of list){
  if(tab==='outfits'&&(!section||section.dataset.slot!==item.slot)){
   section=document.createElement('section');section.className='outfit-section';section.dataset.slot=item.slot;section.setAttribute('aria-labelledby','heading-'+item.slot);
   const heading=document.createElement('div');heading.className='section-heading';const h=document.createElement('h2');h.id='heading-'+item.slot;h.textContent=item.slot==='top'?'Tops':'Bottoms';const selected=document.createElement('span');selected.textContent=ITEMS.find(x=>x.id===state[item.slot])?.name||'Choose your '+item.slot;heading.append(h,selected);section.append(heading);
   const grid=document.createElement('div');grid.className='garment-grid';section.append(grid);box.append(section);
  }
 ...
 renderFolio({destination,tab,extras:EXTRAS.filter(x=>state.extras.includes(x.id)),canUndo:outfitHistory.length>0,thumbnail});
}
...
function selectItem(id){
 ...
 else if(tab==='outfits'||tab==='tops'||tab==='bottoms')state=equipClothing(state,id);
...
function play(type){if(!motion)return;if(['relaxed','little','tada'].includes(type)){state.pose=type;motion.setPose(type);persist();if(type!=='relaxed')chime();return}motion.play(type);if(type==='wave'){say('Hello, you ♡');chime('happy')}if(type==='twirl')chime('happy');if(type==='dance'){say('A little happy dance!');chime('happy')}if(type==='love'){say('So loved, so lucky ♡');chime('happy')}if(type==='camera'){say('My good side? Every side ♡');chime('happy')}if(type==='delight'){say('Oh, I love this one ♡');chime()}if(type==='reveal'){say('Ta-da! What do you think?');chime('happy')}if(type==='kiss'){say('This one’s for you ♡');chime('happy')}if(type==='wiggle'){say('A very good day for a wiggle!');burst('✧');chime('happy')}if(type==='boop'){say('Mmm… more little boops, please ♡',5500);burst();chime('happy')}}
$$('[data-action]').forEach(b=>b.onclick=()=>play(b.dataset.action));$('#surprise').onclick=()=>{rememberLook();state=randomLook(state);applyCharacter().then(ready=>{if(ready)play('dress')});renderItems();persist();say('A little surprise, just for you.');chime('happy')};
...
$('#save-look').onclick=()=>{if(!character)return;const look={id:crypto.randomUUID(),name:lookName(state),date:new Date().toISOString(),state:structuredClone(state),image:captureScene(300,380).toDataURL('image/jpeg',.78)};looks.unshift(look);looks=looks.slice(0,24);try{localStorage.setItem(LOOKS,JSON.stringify(looks));toast('Your look is saved ♡');chime('happy')}catch{toast('This browser couldn’t save your look.')}};
function renderLooks(){const list=$('#saved-list');list.replaceChildren();if(!looks.length){const p=document.createElement('p');p.className='empty-looks';p.textContent='A favorite outfit belongs here. Try “Save look”.';list.append(p);return}for(const look of looks){const row=document.createElement('div');row.className='saved-row';const img=document.createElement('img');img.src=look.image||thumbnail(look.state.outfit!=='base'?look.state.outfit:look.state.top||look.state.bottom||'base');img.alt='';row.append(img);const meta=document.createElement('div');meta.className='saved-meta';const title=document.createElement('strong');title.textContent=look.name;const sub=document.createElement('small');sub.textContent=PLACES.find(p=>p.id===look.state.place).name;meta.append(title,sub);row.append(meta);const wear=document.createElement('button');wear.textContent='Wear';wear.onclick=()=>{state=sanitizeState(look.state);applyCharacter().then(ready=>{if(ready)play('dress')});setPlace(state.place);renderItems();persist();$('#saved-dialog').close()};const del=document.createElement('button');del.textContent='×';del.setAttribute('aria-label','Remove saved '+look.name);del.onclick=()=>{looks=looks.filter(l=>l.id!==look.id);try{localStorage.setItem(LOOKS,JSON.stringify(looks))}catch{}renderLooks()};const rename=document.createElement('button');rename.textContent='Rename';rename.onclick=()=>{const input=document.createElement('input');input.className='look-name-input';input.value=look.name;input.maxLength=60;input.setAttribute('aria-label','Look name');title.replaceWith(input);input.focus();input.select();const finish=()=>{look.name=input.value.trim()||look.name;try{localStorage.setItem(LOOKS,JSON.stringify(looks))}catch{}renderLooks()};input.onchange=finish;input.onkeydown=e=>{if(e.key==='Enter')finish();if(e.key==='Escape')renderLooks()}};row.append(wear,rename,del);list.append(row)}}
...
async function applyCharacter(){
 ...
 eyeSkinTint?.setColor(state.colors.base_primary);motion?.setGentle(state.gentle);if(motion&&motion.selectedPose!==state.pose)motion.setPose(state.pose);
...
function captureScene(width,height){
 ...
 const background=backgroundPhoto; if(background?.complete)drawCover(ctx,background,width,height);
 const ratio=renderer.getPixelRatio(),size=renderer.getSize(new THREE.Vector2()),aspect=camera.aspect,z=camera.position.z,zoom=camera.zoom;
 try{renderer.setPixelRatio(1);renderer.setSize(width,height,false);camera.aspect=width/height;camera.zoom=1;const f=Math.tan(THREE.MathUtils.degToRad(camera.fov/2));camera.position.z=Math.max(4.65/(2*f),3.35/(2*f*camera.aspect));camera.lookAt(0,2.05,0);camera.updateProjectionMatrix();renderer.render(scene,camera);ctx.drawImage(canvas,0,0,width,height);}
 finally{renderer.setPixelRatio(ratio);renderer.setSize(size.x,size.y,false);camera.aspect=aspect;camera.position.z=z;camera.zoom=zoom;camera.lookAt(0,2.05,0);camera.updateProjectionMatrix();renderer.render(scene,camera);}
 return out;
}
let photoFraming='portrait';
$('#photo').onclick=()=>{if(!character)return;document.body.classList.add('photo-mode');$('#photo-return').focus();};
$('#photo-return').onclick=()=>{document.body.classList.remove('photo-mode');$('#photo').focus();};
$$('[data-framing]').filter(b=>b.tagName==='BUTTON').forEach(b=>b.onclick=()=>{photoFraming=b.dataset.framing;document.body.dataset.photoFraming=photoFraming;$$('[data-framing]').filter(x=>x.tagName==='BUTTON').forEach(x=>x.setAttribute('aria-pressed',String(x===b)))});
$('#photo-capture').onclick=()=>{if(!character)return;try{photoSnapshot=captureScene(photoFraming==='portrait'?1200:1600,photoFraming==='portrait'?1600:1067);$('#photo-preview').src=photoSnapshot.toDataURL('image/png');$('#photo-caption').value='';$('#photo-dialog').showModal();chime()}catch(error){console.error(error);toast('The photo couldn’t be made. Please try again.')}};
$('#download-photo').onclick=()=>{if(!photoSnapshot)return;const out=document.createElement('canvas');out.width=photoSnapshot.width;out.height=photoSnapshot.height+88;const c=out.getContext('2d');c.fillStyle='#fff8ef';c.fillRect(0,0,out.width,out.height);c.drawImage(photoSnapshot,20,20,out.width-40,photoSnapshot.height-20);c.fillStyle='#174859';c.font='28px Georgia';c.textAlign='center';c.fillText($('#photo-caption').value||'A little moment with you ♡',out.width/2,out.height-33,out.width-70);out.toBlob(blob=>{if(!blob)return;const a=document.createElement('a');const url=URL.createObjectURL(blob);a.href=url;a.download='beluga-little-keepsake.png';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)},'image/png')};
connectFolio({
 navigate(id){if(id==='looks'){$('#saved-button').click();return}expandWardrobe();destination=id;if(id==='wardrobe')switchTab(wardrobeTab);else if(id==='room')switchTab('room');else renderItems();},
 undo(){if(!outfitHistory.length)return;state=sanitizeState(outfitHistory.pop());applyCharacter();setPlace(state.place);persist();renderItems();},
 front(){targetYaw=-.13;},
